    ...
    def post(self, request):
        ...
        try:
            ...
            if check_user is None:
                response[
                    "message"
                ] = "invalid username , try login in with you email address"
                raise Exception("invalid username not found")

            if not Profile.objects.filter(username=check_user).first().is_verified():
            ...
            if profile is None:
                response["message"] = "Profile not found for the user"
